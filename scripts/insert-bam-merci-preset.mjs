...
function assetUrl(bucket, objectPath) {
  return `${BASE}/storage/v1/object/public/${bucket}/${ORPHAN}/${objectPath}`;
}
...
const presetData = {
  ...
  profile: {
    display_name: "Merci",
    bio: "",
    avatar_url: assetUrl("profiles", "avatar.jpg"),
    banner_url: null,
  },
  settings: {
    layout: "classic",
    ...
    gradient_colors: ["#000000", "#0a0a0a", "#111111"],
    glassmorphism: true,
    neon_glow: false,
    border_radius: 16,
    profile_opacity: 85,
    ...
    background_video_url: assetUrl("backgrounds", "background.mp4"),
    particle_effect: null,
    overlay_opacity: 40,
    vignette: false,
    noise_texture: false,
    music_url: assetUrl("music", "track.mp3"),
    music_title: "Exitin",
    music_autoplay: true,
    music_loop: true,
    music_volume: 50,
    cursor_effect: "glow",
    cursor_image_url: assetUrl("profiles", "cursor.png"),
    ...
    enter_gate_background_color: "#000000",
    enter_gate_background_image_url: assetUrl("backgrounds", "enter-gate.png"),
    enter_gate_background_video_url: null,
    enter_gate_gradient_colors: ["#000000", "#0a0a0a", "#111111"],
...
const bundle = {
  recovered_from: {
    ...
    display_name: "Merci",
    user_id: ORPHAN,
    recovery_source: "orphaned_storage_after_account_deletion",
  ...
  recovery_notes: {
    confirmed: [
      "display_name Merci",
      "username was 444, uid 4",
      "avatar, cursor, background video, enter-gate image, music track",
      "music title Exitin (ID3: Cece Natalie — Miss Behaves)",
      "one custom link icon existed (gun/heart silhouette) but link title/url were deleted with the account",
    ],
    not_recoverable_without_backup: [
      "bio text",
      "exact layout/colors/effects beyond asset-derived defaults",
      "link URLs and titles",
      "badges",
      "discord widget config",
    ],
    orphaned_link_icon_url: assetUrl("profiles", "link-icons/1781565433033.png"),
  },
...
// Insert into bam's profile_presets unless --dry-run
const dryRun = process.argv.includes("--dry-run");
const { data: bam } = await sb.from("profiles").select("id, username").eq("username", "bam").maybeSingle();
if (!bam) {
  console.error("bam profile not found");
  process.exit(1);
}
...
if (existing) {
  console.log("Preset already exists:", existing.id);
  if (!dryRun) {
    const { error } = await sb
      .from("profile_presets")
      .update({
        preset_data: presetData,
        thumbnail_url: presetData.settings.background_video_url,
      })
      .eq("id", existing.id);
    if (error) {
    ...
    console.log("Updated existing preset", existing.id);
  }
} else if (!dryRun) {
  ...
  console.log("Inserted preset for bam:", inserted.id);
} else {
  console.log("dry-run: would insert preset for bam");
}
